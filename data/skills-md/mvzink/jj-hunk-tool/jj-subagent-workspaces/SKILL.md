...
- **Multiple agents in one shared workspace.** Isolated workspaces exist precisely to avoid this. When agents must share a single working copy, serialize history-mutating commands so only one mutating actor runs at a time — concurrent mutation from the same `@` produces divergent change IDs: jj detects the concurrent modification and resolves it automatically, but you're left reconciling the divergence yourself.
